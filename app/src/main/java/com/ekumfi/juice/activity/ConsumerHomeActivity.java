...
import com.ekumfi.juice.fragment.CartListFragment;
import com.ekumfi.juice.fragment.ChatIndexFragment;
import com.ekumfi.juice.fragment.ConsumerSettingsFragment;
...
public class ConsumerHomeActivity extends PermisoActivity implements ConsumerSettingsFragment.Callbacks {
    ...
    private BottomNavigationView.OnNavigationItemSelectedListener mOnNavigationItemSelectedListener
        ...
        public boolean onNavigationItemSelected(@NonNull MenuItem item) {
            ...
            if (itemId == R.id.search_product) {
                ...
                fragment = new ConsumerSettingsFragment();
                loadFragment(fragment);
                return true;
            }
            return false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        guidSocket = Socket
                .Builder.with(keyConst.GUID_WS_URL)
                .build();
        guidSocket.connect();

        guidSocket.onEvent(Socket.EVENT_OPEN, new Socket.OnEventListener() {
            @Override
            public void onMessage(String event) {
                                    ...
                                    if (finalJsonResponse.getJSONObject("data").has("guid")) {
                                        if (!PreferenceManager.getDefaultSharedPreferences(activeActivity).getString("com.ekumfi.juice" + GUID, "").equals("") && !finalJsonResponse.getJSONObject("data").getString("guid").equals(PreferenceManager.getDefaultSharedPreferences(activeActivity).getString("com.ekumfi.juice" + GUID, ""))) {
                                            ...
                                            builder.setPositiveButton(android.R.string.yes, (dialog, which) -> {
                                                Const.clearAppData(activeActivity);
                                            });
                                            builder
                                                    .setCancelable(false)
                                                    .show();
                ...
                try {
                    ...
                    if (Const.isNetworkAvailable(homeactivity)) {
        ...
        guidSocket.onEvent(Socket.EVENT_RECONNECT_ATTEMPT, new Socket.OnEventListener() {
            @Override
            public void onMessage(String event) {
                Log.d("mywebsocket2", "reconnecting");
            }
        });
        guidSocket.onEvent(Socket.EVENT_CLOSED, new Socket.OnEventListener() {
            @Override
            public void onMessage(String event) {
        ...
        Realm.getInstance(RealmUtility.getDefaultConfig(ConsumerHomeActivity.this)).executeTransaction(realm -> {
            RealmResults<RealmCart> realmCarts = realm.where(RealmCart.class)
                    .distinct("seller_id")
                    .equalTo("consumer_id", PreferenceManager.getDefaultSharedPreferences(getApplicationContext()).getString("com.ekumfi.juice" + "CONSUMER_ID", ""))
                    .findAll();
            for (RealmCart realmCart : realmCarts) {
                FirebaseMessaging.getInstance().subscribeToTopic(realmCart.getSeller_id() + PreferenceManager.getDefaultSharedPreferences(getApplicationContext()).getString("com.ekumfi.juice" + "CONSUMER_ID", ""))
                        .addOnCompleteListener(task -> {
                            String msg = "successfully subscribed";
                            if (!task.isSuccessful()) {
...
//                                    new broadcastWithFirebase().execute();
            ...
            case 1815:
                switch (resultCode) {
                    case Activity.RESULT_OK:
                        if (data != null) {
                            SellerProductsFragment.productMaterialDialog.setProduct_id(data.getStringExtra("PRODUCT_ID"));

                            SellerProductsFragment.productMaterialDialog.name_textview.setText(data.getStringExtra("NAME"));
                            SellerProductsFragment.productMaterialDialog.name_textview.setError(null);
                        }
                        break;
    ...
    public static void versionCheck(Context context) {
        try {
            StringRequest stringRequest = new StringRequest(
                    Request.Method.GET,
                    keyConst.API_URL + "student-project-info",
                    response -> {
                        if (response != null) {
                            try {
                                JSONObject jsonObject = new JSONObject(response);
                                if (!jsonObject.getString("version").equals(InitApplication.versionName)) {
...
//        realm.createOrUpdateAllFromJson(RealmDrawingCoordinate.class, responseJson.getJSONArray("drawing_coordinates"));
    ...
    public static void retriev_current_registration_token(Context context, String confirmation_token) {
        Realm.init(context);
        RealmConsumer realmConsumer = Realm.getInstance(RealmUtility.getDefaultConfig(context)).where(RealmConsumer.class).findFirst();
