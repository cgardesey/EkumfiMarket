...
public class RiderProviderAccountFragment6 extends MultiPickerWrapperSupportFragment {
    ...
    public RoundedImageView drivers_licence_image;
    Context mContext;
    LinearLayout controls;
    TextView image_not_set;
    public static File reverse_license_image_file = null;
    MultiPickerWrapper.PickerUtilListener multiPickerWrapperListener = new MultiPickerWrapper.PickerUtilListener() {
        ...
        public void onImagesChosen(List<ChosenImage> list) {
            ...
            drivers_licence_image.setImageBitmap(BitmapFactory.decodeFile(imagePath));

            reverse_license_image_file = new File(list.get(0).getOriginalPath());
        }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        final View rootView = inflater.inflate(R.layout.fragment_rider_provider_account6, container, false);
        drivers_licence_image = rootView.findViewById(R.id.reverse_drivers_licence_image);
        ...
        drivers_licence_image.setOnClickListener(view -> {
            if (drivers_licence_image.getDrawable() == null) {
                //Toast.makeText(mContext, getString(R.string.image_not_set), Toast.LENGTH_SHORT).show();
            } else {
                PictureActivity.idPicBitmap = ((RoundedDrawable) drivers_licence_image.getDrawable()).getSourceBitmap();
    ...
    public void init() {
        if (RiderProviderAccountActivity.realmProvider != null) {
            String picture = RiderProviderAccountActivity.realmProvider.getDrivers_licence_reverse_image_url();
            boolean pictureExists = picture != null;
            if (pictureExists) {
                Glide.with(getContext())
                        .load(RiderProviderAccountActivity.realmProvider.getDrivers_licence_reverse_image_url())
                        .apply(new RequestOptions().centerCrop())
                        .apply(RequestOptions.signatureOf(ApplicationVersionSignature.obtain(getContext())))
                        .into(drivers_licence_image);
            } else {
                drivers_licence_image.setImageBitmap(null);
            }
        }
    }
